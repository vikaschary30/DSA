...
//             1) optimal - TC:O(1(for operations)) SC:O(capacity(for storing nodes+frequency lists)) Your LFUCache uses a HashMap for key→node lookup and a frequency→doubly linked list map to track usage counts, always evicting the least frequently used (and least recently used on ties) node in O(1).
