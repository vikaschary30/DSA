...
//                                  1) brute force - TC : O((max-min)*n) Try every day from the earliest bloom (min) to the latest bloom (max), and for each day, count how many bouquets can be formed; the first day with at least m bouquets is the answer.
//                                  2) optimal - TC : O(log(max-min)*n) Search for the smallest day between min and max such that at least m bouquets of size k can be made.
